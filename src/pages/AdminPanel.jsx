...
function AdminPanel() {
  ...
  const [paymentRef, setPaymentRef] = useState('') // payment reference input
  const [selectedUser, setSelectedUser] = useState(null) // user being marked as paid
  ...
  async function markAsPaid(userId) {
    if (!paymentRef.trim()) {
      alert('Please enter the Paystack payment reference')
      return
    }
    ...
    try {
      ...
      setUsers(users.map(user => 
        user.id === userId ? { ...user, paid: true, paidUntil: paidUntil, paymentReference: paymentRef.trim() } : user
      ))
  ...
  async function markAsUnpaid(userId) {
    ...
    try {
      ...
      setUsers(users.map(user => 
        user.id === userId ? { ...user, paid: false, paidUntil: null, paymentReference: null, paidAt: null } : user
      ))
  ...
  const pendingVerification = users.filter(u => !user.verified && u.accountType !== 'client').length
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {selectedUser && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-slate-800 border-slate-700 rounded-2xl p-6 max-w-md w-full">
              <h3 className="text-xl font-bold mb-2">Mark as Paid</h3>
              <p className="text-slate-400 text-sm mb-4">
                Enter the Paystack payment reference for {selectedUser.email}
              </p>
              <input
                type="text"
                value={paymentRef}
                onChange={(e) => setPaymentRef(e.target.value)}
                placeholder="e.g. PAY_1234567890ABC"
                className="w-full bg-slate-900 border-slate-600 rounded-lg px-4 py-3 text-white mb-4 focus:outline-none focus:ring-2 focus:ring-blue-600"
              />
              <p className="text-slate-500 text-xs mb-4">
                You’ll find this in Paystack → Payments → Transactions → Reference
              </p>
              <div className="flex gap-3">
                <button
                  onClick={() => markAsPaid(selectedUser.id)}
                  disabled={updating === selectedUser.id}
                  className="flex-1 bg-blue-600 hover:bg-blue-700 px-4 py-3 rounded-lg font-semibold transition disabled:opacity-50"
                >
                  {updating === selectedUser.id ? 'Saving...' : 'Confirm Payment'}
                </button>
                <button
                  onClick={() => {
                    setSelectedUser(null)
                  ...
                  className="flex-1 bg-slate-700 hover:bg-slate-600 px-4 py-3 rounded-lg font-semibold transition"
                >
                  Cancel
                </button>
              </div>
                        ...
                        <div>
                          ...
                          {user.paymentReference && (
                            <p className="text-slate-500 text-xs font-mono mt-1">
                              Ref: {user.paymentReference.slice(0, 12)}...
                            </p>
                          )}
                        </div>
                      ) : (
        ...
        <div className="mt-8 bg-slate-800 border-slate-700 rounded-2xl p-5">
          ...
          <ol className="text-slate-300 text-sm space-y-2 list-decimal list-inside">
            <li>Provider pays R10 via Paystack and sends you receipt + Firebase UID + Paystack reference on WhatsApp</li>
            <li>Find provider in this table using their email or UID</li>
            <li>Click <span className="text-blue-400 font-semibold">“Mark Paid”</span> → paste Paystack reference → Confirm</li>
          ...
          <p className="text-slate-400 text-xs mt-4">
            Paystack Reference: Found in Paystack → Payments → Transactions → Click transaction → Reference
          </p>
        </div>
      </div>
    </div>
